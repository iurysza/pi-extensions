...
describe("cache hit prediction", () => {
  test("bounds the old lane prefix by the current prompt", () => {
    ...
    const prediction = predictCacheHit(history, {
      provider: "openai",
      api: "openai-responses",
      model: "gpt-test",
      thinkingLevel: "low",
    }, 100_000);
    assert.equal(prediction.estimatedCacheTokens, 25_000);
    assert.equal(prediction.percent, 25);
  });
  ...
  test("reports an unseen destination as cold", () => {
    ...
    assert.equal(prediction.hasLaneHistory, false);
    assert.equal(prediction.estimatedCacheTokens, 0);
    assert.equal(prediction.percent, 0);
  });
});
