...
| Raw Cursor SDK `AbortError` containment | **Implemented** | `src/cursor-sdk-process-error-guard.ts` suppresses only active-turn abort errors with Cursor SDK stack provenance; focused process-guard tests cover DOMException/Error, uncaught/rejection, inactive, and provenance-negative paths. |
